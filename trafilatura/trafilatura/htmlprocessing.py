...
def tree_cleaning(tree, options):
    ...
    if options.images is True:
        ...
        cleaning_list = [e for e in cleaning_list if e
                         not in ('figure', 'picture', 'source', 'iframe', 'embed', 'video')]
        #ddddd
...
def delete_by_link_density(subtree, tagname, backtracking=False, favor_precision=False):
    ...
    for elem in uniquify_list(deletions):
        #dddddd
        if elem.getparent().tag not in ['graphic', 'figure', 'picture']:
            try:
...
def convert_tags(tree, options, url=None):
    ...
    for elem in tree.iter('blockquote', 'br', 'del', 'details', 'dl', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'hr', 'ol', 'pre', 'q', 's', 'strike', 'ul'):
        ...
        # head tags + delete attributes
            ...
            class_element =  elem.get('class') if elem.get('class') else ""
            id_element = elem.get('id') if elem.get('id') else ""
            elem.attrib.clear()
            # attributes_to_remove = [attr for attr in elem.attrib if attr != 'position']
            # # Supprimer chaque attribut non désiré
            # for attr in attributes_to_remove:
            #     del elem.attrib[attr]
            elem.set('rend', elem.tag)
            elem.set('class', class_element)
            elem.set('id', id_element)
            elem.set('position', position)
            elem.tag = 'head'
        # br → lb
