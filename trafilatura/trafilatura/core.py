...
def handle_formatting(element, options):
    ...
    processed_element.set('class', element.get('class') if element.get('class') else "")
    processed_element.set('id', element.get('id') if element.get('id') else "") 
    processed_element.set('position', element.get('position'))
...
def handle_paragraphs(element, potential_tags, options):
    ...
    class_element =  element.get('class') if element.get('class') else ""
    id_element = element.get('id') if element.get('id') else ""
    ...
    if len(element) == 0:
        ...
        if processed_element is not None:
            processed_element.set('class', class_element)
            processed_element.set('id', id_element)
            processed_element.set('position', position)
            return processed_element
        return None
    # children
    processed_element = Element(element.tag)
...
def handle_image(element):
    ...
    elif is_image_file(element.get('srcset')):
        try:
            # Regex pattern to match URLs
            pattern = r'https?://[^\s,]+'
            urls = re.findall(pattern, element.get('srcset'))
            if not urls:
                print("No URL found")
            processed_element.set('src', urls[0])
    ...
    # List of potential image source attributes
    image_attributes = [
        'data-src', 'src', 'data-ezsrc', 'data-lazy-src', 'data-original',
        'data-srcset', 'data-ezsrcset'
    ]
    
    # Check each attribute and set the first valid one found
    for attr in image_attributes:
        if attr in element.attrib and is_image_file(element.get(attr)):
            processed_element.set('src', element.get(attr))
            break
...
def filter_links(links_around_middle):
    seen_urls = set()
    filtered_links = []

    for link in links_around_middle:
        src = link.get('src', '')  
...
def extract_links_positions(html):
    h1_match = re.search(r'<h1.*?>', html)
    if h1_match:
        h1_position = h1_match.start()
        h1_position = h1_position * 0.75
        h1_position = math.floor(round(h1_position))
        # Extract links only after the <h1> tag
        links_after_h1_a = re.finditer(r'<a\s+(?:[^>]*?\s+)?href="([^"]*)"\s+[^>]*?position="(\d+)"|<a\s+(?:[^>]*?\s+)?position="(\d+)"\s+[^>]*?href="([^"]*)"', html[h1_position:])
        
        # Combine all types of links
        all_links_after_h1, all_embed = [], []
        for match in links_after_h1_a:
            all_links_after_h1.append({'src': match.group(1) if match.group(1) else match.group(4), 'x': match.start(), 'y': match.group(2) if match.group(2) else match.group(3), 'type': 'a'})

        links_after_h1_iframe_iter = re.finditer(r'<iframe\s+(?:[^>]*?\s+)?(?:(src|data-src)="([^"]*)"|position="(\d+)").*?(?:(src|data-src)="([^"]*)"|position="(\d+))"', html[h1_position:])
        all_embed = []  # Initialize list to store iframe elements
        for _ in range(5):  # Process only the first 5 elements
            try:
                match = next(links_after_h1_iframe_iter)
                all_embed.append({'src': match.group(2) if match.group(1) in ['src', 'data-src'] else match.group(5), 
                                  'x': match.start(), 
                                  'y': match.group(3) if match.group(3) else match.group(6), 
                                  'type': 'iframe'
                                  })
            except StopIteration:
                break

        # Process <embed> tags if there are at least 5 elements
        links_after_h1_embed_iter = re.finditer(r'<embed\s+(?:[^>]*?\s+)?((src|data-src)="([^"]*)").*?position="(\d+)"|<embed\s+(?:[^>]*?\s+)?position="(\d+)".*?((src|data-src)="([^"]*)")', html[h1_position:])
        for _ in range(5):  # Process only the first 5 elements
            try:
                match = next(links_after_h1_embed_iter)
                all_embed.append({'src': match.group(3) if match.group(3) else match.group(8), 'x': match.start(), 'y': match.group(4) if match.group(4) else match.group(5), 'type': 'embed'})
            except StopIteration:
                break

        # Process <embed> tags if there are at least 5 elements
        video_iter = re.finditer(r'<video\s+[^>]*?id="([^"]*)".*?data-media-id="([^"]*)".*?data-player="([^"]*)".*?position="(\d+)".*?>|<video\s+.*?position="(\d+)".*?id="([^"]*)".*?data-media-id="([^"]*)".*?data-player="([^"]*)".*?>', html[h1_position:])
        for _ in range(5):  # Process only the first 5 elements
            try:
                match = next(video_iter)
                all_embed.append({
                    'src': match.group(1) if match.group(1) else match.group(6),
                    'x': match.start(),
                    'y': match.group(4) if match.group(4) else match.group(5),
                    'type': 'jwplayer'
                })
            except StopIteration:
                break
            
        # Process <video> tags if there are at least 5 elements
        links_after_h1_video_iter = re.finditer(r'<video\s+(?:[^>]*?\s+)?(?:src|data-src)="([^"]*)".*?position="(\d+)"|<video\s+(?:[^>]*?\s+)?position="(\d+)".*?(?:src|data-src)="([^"]*)"', html[h1_position:])
        for _ in range(5):  # Process only the first 5 elements
            try:
                match = next(links_after_h1_video_iter)
                all_embed.append({'src': match.group(1) if match.group(1) else match.group(4), 'x': match.start(), 'y': match.group(2) if match.group(2) else match.group(3), 'type': 'video'})
            except StopIteration:
                break
            
        script_iter = re.finditer(r'jwVideoId\\":\\"([^\\"]+)', html)
        for match in script_iter:
            all_embed.append({
                'src': match.group(1),
                    'x': match.start(),
                    'y': match.end(),
        ...
        script_contents = re.findall(r'<script[^>]*\btype="application/ld\+json"[^>]*\bposition="(\d+)"[^>]*>([\s\S]*?)<\/script>',  html)
        for position, content in script_contents:
            match = re.search(r'"embedUrl":\s*"([^"]+)"', content)
            if match:
                embed_url = match.group(1)
                all_embed.append({'src': embed_url, 'x': match.start(), 'y': position, 'type': 'brightcove' })
            else:
                pass
            
        pattern_dailymotion = re.compile(r'<div [^>]*?data-id="([^"]+)"[^>]*?data-provider="dailymotion"[^>]*?position="(\d+)"[^>]*?>|<div [^>]*?position="(\d+)"[^>]*?data-id="([^"]+)"[^>]*?data-provider="dailymotion"[^>]*?>')
        matches_dailymotion = pattern_dailymotion.finditer(html)
        if matches_dailymotion:
            for match in matches_dailymotion:
                all_embed.append({'src': "https://www.dailymotion.com/video/"+match.group(1) if match.group(1) else match.group(4), 'x': match.start(), 'y': match.group(2) if match.group(2) else match.group(3), 'type': 'dailymotion' })
        # Calculate the middle index based on all links after <h1>
        middle_index = len(all_links_after_h1) // 2

        # Extract the 10 links around the middle (5 before and 5 after)
        links_around_middle = all_links_after_h1[max(0, middle_index - 25):middle_index + 26]
        links_around_middle = links_around_middle + (all_embed)
        
        links_around_middle = filter_links(links_around_middle)
        return links_around_middle
    else:
        # Extract all links from the entire HTML if no <h1> tag is found
        all_links = re.findall(r'(?:(?:<a\s+(?:[^>]*?\s+)?href="([^"]*)")|(?:<iframe\s+(?:[^>]*?\s+)?src="([^"]*)")|(?:<embed\s+(?:[^>]*?\s+)?src="([^"]*)")|(?:<video\s+(?:[^>]*?\s+)?src="([^"]*)"))', html)
        merged_array = [link for link in sum(all_links, ()) if link]  # Combine all non-empty links
        links_around_middle = []
        for rrr in merged_array:
            links_around_middle.append({'src': rrr, 'x': None, 'y': None, 'type': 'a'})
        return links_around_middle
...
def process_comments_node(elem, potential_tags, options):
    ...
    if elem.tag in potential_tags:
        ...
        if processed_element is not None:  # and processed_element.text not in COMMENTS_BLACKLIST:
            ...
            attributes_to_remove = [attr for attr in elem.attrib if attr not in ['position','class','id']]
            # Supprimer chaque attribut non désiré
            for attr in attributes_to_remove:
                del processed_element.attrib[attr]
...
def bare_extraction(filecontent, url=None, no_fallback=False,  # fast=False,
    ...
    try:
        ...
        if output_format != 'txt':
            ...
            if document.url is not None and document.date is not None:
        ...
        #ddddd
        tree = add_position_to_tree_elements(tree)
        
        html_convert = tostring(tree, pretty_print=True, encoding='unicode')
        merged_array_link = extract_links_positions(html_convert)
    ...
    return document, merged_array_link
...
def extract(filecontent, url=None, record_id=None, no_fallback=False,
    ...
    # extraction
    try:
        document, merged_array_link  = bare_extraction(
    ...
    # return
    return determine_returnstring(document, output_format, include_formatting, tei_validation), merged_array_link
...
def add_position_to_tree_elements(tree):
     #dddddddd
    id_counter = 0 # Incrémenter l'ID pour le prochain élément
    for elm in tree.iter('*'):
        elm.set('position', str(id_counter))
        id_counter += 1  # Incrémenter l'ID pour le prochain élément
    return tree